...
Run `mise run generate:update` to discover published action versions and regenerate the sources.
Discovery follows each registry entry's `version_pattern`, preserves existing versions, and succeeds
when no updates are needed. Run `mise run test` and `mise run workflows` to validate the result.
